...
var operationRequirementContract = &Interface{
	OpenBindings: "0.2.0",
...
func operationRequirementCandidate(bindingSpec string, output JSONSchema) *Interface {
	if output == nil {
		output = map[string]any{"$ref": "#/schemas/CreateOutput"}
	}
	return &Interface{
...
func (b *operationRequirementBinding) BindingSpecs() []BindingSpecInfo {
	return []BindingSpecInfo{{BindingSpec: b.bindingSpec}}
}
...
func TestResolveOperationRequirementRefusesIncompatibleSchema(t *testing.T) {
	...
	if len(resolution.Assessments) != 1 ||
		...
		resolution.Assessments[0].Issues[0].Kind != CompatibilityOutputIncompatible {
