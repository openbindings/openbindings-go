...
func TestInterfaceValidate_SatisfiesRoleMustExistInRoles(t *testing.T) {
	...
	if !containsProblem(err, `operations["a"].satisfies[0].role: references unknown role "nonexistent"`) {
...
func TestInterfaceValidate_SecurityRefMustExist(t *testing.T) {
	...
	if !containsProblem(err, `bindings["op.api"].security: references unknown security "nonexistent"`) {
