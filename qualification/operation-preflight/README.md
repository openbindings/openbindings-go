# Optional operation preflight: implementation qualification

> The operation qualified here was named preparation (`PrepareOperation`,
> `PrepareBinding`, `BindingPreparer`) when this record was written. The same
> day it was renamed to preflight and its documented contract was reduced to
> the preflight signal contract in [`PREFLIGHT.md`](../../PREFLIGHT.md). The
> prose below uses the current names; the behavior it records is unchanged.

Implemented locally on 2026-09-21 over SDK baseline
`09bad457a5363bca90cf507a7acded7bcb966494`, branch
`feat/value-migration-rulings`. The accepted [preflight contract](../../PREFLIGHT.md)
keeps the existing public signatures and automatic preflight/resolution lane.
...
the binding chooses useful setup and owns its reusable state. Preflight may
...
`design/sdk-preparation` directory (its historical name).

## Implementation and checks

The functional correction is in OpenAPI preflight: required load, edition,
...
Explicit preflight never calls the application's context resolver or dispatches
...
- repeated early preflight retaining embedded analysis, no request before
...
- explicit preflight reporting requirements without resolving or invoking,
  followed by normal automatic preflight/resolution;
- required preflight failure preventing execution without permanently
  disabling a later invocation.

Existing tests cover absent preflighters, alias and pinned-binding selection,
...
- Focused new/existing preflight regressions: passed with the race detector.
...
the preflight wording changes in isolated worktrees. Fixtures are unchanged.
...
The checked-in `BenchmarkOperationPreflight` (then `BenchmarkOperationPreparation`; the
[raw output](benchmark.txt) keeps the recorded names) compares a fresh adapter per
...
| Early `PreflightOperation` | 235.28 | 414.60 | 640.44 |
...
still load separately during preflight and invocation; the concurrency test
...
- `interfaces-preparation`: binding/operation preflight descriptions and
  requested-operation refusal wording; removes unconditional preflight
...
- `project-preparation`: records the accepted decision and supersedes the
  no-I/O preflight premise of the historical cache ruling, without
