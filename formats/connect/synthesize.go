...
func convertToInterface(disc *discovery, sourceLocation string, onWarning func(synthesize.SynthesizerWarning)) (openbindings.Interface, error) {
	...
	for _, svc := range disc.services {
		...
		for _, method := range methods {
			// openbindings.connect@1 incorporates the gRPC binding's accepted
			// protobuf schema range. Excluded methods remain visible through
			// durable coverage accounting but must not be emitted as bindings
			// invocation is statically guaranteed to refuse.
...
func newSchemaWalker(direction schemaDirection, onWarning func(synthesize.SynthesizerWarning), path string) *schemaWalker {
	return &schemaWalker{
		...
		baseID:    "urn:openbindings:generated:connect:" + path,
		onWarning: onWarning,
		path:      path,
	}
}
