...
// repository's binding-specs/asyncapi fixtures (ASYNC-D-01..03) through this
...
func bindingSpecCorpusDir(t *testing.T) string {
	...
	dir := filepath.Join(root, "binding-specs", "asyncapi")
...
func judgeCorpusDocument(t *testing.T, raw json.RawMessage) error {
	...
	for name, src := range obiDoc.Sources {
		...
		// Content lane (ASYNC-D-01): a present member — null included —
		// must be the parsed document object or its source text. The
		...
		// Selector lane (ASYNC-D-03): selector is REQUIRED (an omitted
		// selector reaches the invoker as the empty string and is refused by the same
		// grammar); against an embedded artifact the pointer must address
		// an operations-map entry, with Reference Objects resolved through
		// before the operation-object test — exactly as runBinding judges.
		for _, b := range obiDoc.Bindings {
			...
			if !ok {
				return fmt.Errorf("operation %q not in AsyncAPI doc", opID)
			}
			if op.Ref != "" {
				return fmt.Errorf("operation %q is a Reference Object whose target %q does not resolve in the AsyncAPI doc", opID, op.Ref)
			}
		}
	}
	return nil
