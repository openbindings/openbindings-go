...
// and selector grammar/resolution — under the subcorpus README's verdict
...
func judgeCorpusDocument(t *testing.T, raw json.RawMessage) error {
	...
	for name, src := range doc.Sources {
		...
		for _, b := range doc.Bindings {
			...
			entityType, remainder, err := parseSelector(selector)
			if err != nil {
				return err
			}
			if entityType != "tools" {
				return fmt.Errorf("openbindings.mcp@1 selector must use tools/<name>, got %q", selector)
			}
			if pin != nil {
				if _, rerr := resolveSelector(pin, entityType, remainder, BindingSpec); rerr != nil {
					return rerr
				}
			}
		}
