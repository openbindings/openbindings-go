...
//
// A magnitude outside that domain (`1e400`, and the corpus's real
// `600e27371700`) keeps its source text, DELIBERATELY, and this is the one
// place this layer does not follow §10.3.2's table. §10.3.2 resolves such a
// spelling to a float; what a processor then does with it is not §10.3.2's to
// say. §10.2.1.4 states that "The supported range and accuracy depends on the
// implementation", which puts refusing the artifact, carrying ±Inf, and
// clamping to the nearest representable double all inside what the authority
// permits — a choice, not a deduction. §3's no-JSON-image refusal does not
// reach it either: unlike `.inf` and `.nan`, which JSON cannot spell at all,
// `6e27371702` is an ordinary JSON number this number type cannot hold.
// Both engines spell such a scalar as its own text today, so leaving it alone
// is the outcome that moves nothing while the question is open; it is filed
// as F-O1-15 and pinned by name in the shared case table. Underflow is NOT in
...
func radixNumberValue(node *yaml3.Node, digits string, base int) (any, error) {
	...
	if math.IsInf(value, 0) {
		return node.Value, nil // F-O1-15, as in decimalNumberValue
	}
	return value, nil
}
