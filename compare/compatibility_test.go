...
func TestIsOBInterface(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := openbindings.IsOBInterface(tt.v)
			if got != tt.want {
...
func TestCheckInterfaceCompatibility_BooleanFalseSchemas(t *testing.T) {
	...
	iface := func(op openbindings.Operation) *openbindings.Interface {
		return &openbindings.Interface{OpenBindings: "0.1.0", Operations: map[string]openbindings.Operation{"op": op}}
	}

	// input false vs input false → compatible
	if issues := CheckInterfaceCompatibility(iface(openbindings.Operation{Input: false}), iface(openbindings.Operation{Input: false})); len(issues) != 0 {
		t.Fatalf("input false vs false: expected 0 issues, got %+v", issues)
	}
	// input false vs input object → incompatible, both directions
	withInput := iface(openbindings.Operation{Input: map[string]any{"type": []any{"object"}}})
	for name, pair := range map[string][2]*openbindings.Interface{
		"false-vs-object": {iface(openbindings.Operation{Input: false}), withInput},
		"object-vs-false": {withInput, iface(openbindings.Operation{Input: false})},
	} {
	...
	if issues := CheckInterfaceCompatibility(iface(openbindings.Operation{Input: true}), iface(openbindings.Operation{Input: map[string]any{}})); len(issues) != 0 {
		t.Fatalf("input true vs {}: expected 0 issues, got %+v", issues)
	}
	// output false vs output false → compatible
	if issues := CheckInterfaceCompatibility(iface(openbindings.Operation{Output: false}), iface(openbindings.Operation{Output: false})); len(issues) != 0 {
